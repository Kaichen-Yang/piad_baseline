...
from Retinex.network.Math_Module import P, Q
from Retinex.network.decom import Decom
import os
import torchvision
...
class Separate(nn.Module):
    ...
    def separate(self, input_low_img):
    ...
    def forward(self, input_low_img):
        input_low_img = input_low_img[:,0:3,:,:]
        if torch.cuda.is_available():
            input_low_img = input_low_img.cuda()
        with torch.no_grad():
            start = time.time()  
            R, L = self.separate(input_low_img)
            # delete edges has different color
            
            bk_color = R[0,:,9,9]
            
            R[0, :, :6, :] = bk_color.view(1,3,1,1)
            R[0, :, -6:, :] = bk_color.view(1,3,1,1)
            R[0, :, :, :6] = bk_color.view(1,3,1,1)
            R[0, :, :, -6:] = bk_color.view(1,3,1,1)
        ...
        return R, L, p_time # I_enhance, p_time
    ...
    """def run(self, low_img_path):
        ...
        Reflection, illumination, p_time = self.forward(input_low_img=low_img)
        if not os.path.exists(self.opts.output):
            os.makedirs(self.opts.output)
        save_path_R = os.path.join(self.opts.output, file_name.replace(name, "%s_%d_R"%(name, self.opts.ratio)))
        save_path_L = os.path.join(self.opts.output, file_name.replace(name, "%s_%d_L"%(name, self.opts.ratio)))
        np_save_TensorImg(Reflection, save_path_R)
        np_save_TensorImg(illumination, save_path_L)
        print("================================= time for %s: %f============================"%(file_name, p_time))"""

    def run(self, low_img_path, out_img_path):
        
        file_name = os.path.basename(low_img_path)
        name = file_name.split('.')[0]
        low_img = self.transform(Image.open(low_img_path)).unsqueeze(0)
        Reflection, _, p_time = self.forward(input_low_img=low_img)
        np_save_TensorImg(Reflection, out_img_path)
        #print("================================= time for %s: %f============================"%(file_name, p_time))
