...
def define_modelR(opts):
    if opts.R_model == "HalfDnCNNSE":
        from Retinex.network.restoration import HalfDnCNNSE
        model_R = HalfDnCNNSE(opts)
    return model_R

def define_modelL(opts):
    if opts.L_model == "Illumination_Alone":
        from Retinex.network.illumination_enhance import Illumination_Alone
        model_L = Illumination_Alone(opts)
    return model_L

def define_modelA(opts):
    if opts.A_model == "naive":
        from Retinex.network.illumination_adjustment import Adjust_naive
